...
const WhatsAppButton = () => {
  ...
  const phoneNumber = "33784819003"; 
  const message = "Bonjour, je souhaiterais obtenir des informations sur vos services.";
  const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`;

  useEffect(() => {
    const interval = setInterval(() => {
      setShowPulse(true);
      setTimeout(() => setShowPulse(false), 3000); // Show for 3 seconds
  ...
  return (
    <div className="whatsapp-wrapper">
      {/* Message Bubble */}
      <div className={`whatsapp-bubble ${showPulse ? 'show' : ''}`}>
        Contactez-nous !
      </div>
